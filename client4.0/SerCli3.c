...
int config_ssl_stc(SSL_CTX *ctx  ){
    /*
    int err = setup_engine(engine_id);
    if(err == 0){
        return 0;
    }
    */
    err = SSL_CTX_set_cipher_list(ctx, "SM2-WITH-SMS4-SM3");
...
//    加载信任的根证书
    ...
    if(SSL_CTX_use_certificate_file(ctx ,sigCrt,SSL_FILETYPE_PEM) <= 0)
...
int socket_init(const char *ip_addr, int port){
    ...
    stLocalAddr.sin_addr.s_addr = inet_addr(ip_addr);//IP,括号表示本机ip
...
int main(int argc, char *argv[]){
    ...
    //获取地址信息
    if (argc < 10) {
        printf("./%s src_addr ports dst_addr portc Cafile sigCrt sigKey encCrt encKey\n", argv[0]);
        exit(1);
    }
    //监听ip 端口
    char *src_addr == "0.0.0.0";
    src_addr = argv[1];
    int  src_port = strtol(argv[2], NULL, 0);
    dst_addr = argv[3];
    dst_port = strtol(argv[4], NULL, 0);
    ca_file = argv[5];
    sigCrt = argv[6];
    sigKey = argv[7];
    encCrt = argv[8];
    ...
    //创建服务器socket 并监听端口
    int localfd  = socket_init(src_addr , src_port);
