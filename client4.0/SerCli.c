...
//                            printf("1 cli >> me %d %s\n", ret, buf);
                    ...
                    while(rdata){
                        ...
                        if (SSL_connect(ssl) == -1) {
                            ...
                            ShowCerts(ssl);
                            printf("1.Connected with %s encryption\n", SSL_get_cipher(ssl));
                        }
                        ...
                        if(ret > 0){
                            ...
                            if(select(fdmax+1,NULL,&fds,NULL,&timeout)>0){
                                if (FD_ISSET(newthread->ClientFD, &fds)) {
                                    ...
                                    if (ret2 != ret) {
                                        printf("%d发送给客户端失败\n",myid);
//                                        printf("%d me >>- cli failed\n",myid);
                                    }else {
                                        printf("%3d发送给客户端%d的数据",myid,ret2);
//                                        printf("%d me >>- cli %d data\n",myid, ret2);
                                    }
                                }
                            }else{
